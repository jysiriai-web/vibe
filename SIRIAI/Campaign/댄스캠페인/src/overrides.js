...
export const EDITABLE_FIELDS = ['nick', 'link', 'notice', 'contentA', 'schedDate', 'fixedDate', 'soundOk', 'soundSection', 'hashtagOk', 'memo', 'mirror',
  ...
  /* 완료여부 — 이 사람의 캠페인 일이 끝났나(완료/진행중). 정산 표에 있는데 되쓰기가 막혀 있어서
     화면에서 고쳐도 400 으로 튕겼다. 자동 스캔이 안 건드리는 칸이라 잠금 대상은 아니다. */
  'campaignDone',
  /* 정산포함(AT열) — '이 사람에게 돈을 주나' 를 정하는 칸. 팔로워 미달·중복지원처럼
     납품은 하되 지급은 다시 보는 건이 여기서 갈린다. 화이트리스트에 없으면 서버가 400 으로
     되돌리는데, 화면은 이미 바뀐 뒤라 '저장한 줄 알았는데 안 남는' 상태가 된다. */
  'settleInc',
  /* 친구추천 수 — 정산 금액(3,000×N)에 직접 들어간다. 브릿지는 '추천인'(refBy)도 읽지만
     그건 응답시트에서 온 사실이라 여기서 고치지 않는다. 사람이 정하는 건 '몇 건 인정하나'다. */
  'refCount'];
