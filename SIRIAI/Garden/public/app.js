...
// ⚠️ 검수 완료 = 세 검수칸(음원19·음원구간20·해시태그21)이 모두 '준수'일 때만.
//    한 칸이라도 미준수 또는 미확인이면 검수대기 (사용자 지정 — 다 통과해야 완료).
const REV_COLS = [19, 20, 21];
const reviewed = (a) => uploaded(a) && REV_COLS.every((c) => revState(revValOf(a, c)) === 'pass');
const reviewPending = (a) => uploaded(a) && !reviewed(a); // 업로드됐는데 세 칸이 다 준수는 아님(미준수·미확인 있음)
...
function viewUpload(accts) {
  ...
  return `<div class="cards">
      ...
      ${kpi('검수 완료', ofTot(rev.length, up.length || accts.length), { ic: IC.check, sub: '세 칸 모두 준수' })}
